...
def analyze_python_file(file_path: str, project_root: str = None) -> str:
    ...
    try:
        ...
        result += f"  Average function length: {analysis['avg_function_length']:.1f} lines\n"
...
def analyze_directory_code(directory: str, project_root: str = None, file_extensions: List[str] = None) -> str:
    """
    ...
    try:
        ...
        stats_by_ext = defaultdict(lambda: {
            'count': 0, 'total_lines': 0, 'total_size': 0,
            'functions': 0, 'classes': 0, 'files': []
        })
...
def find_code_patterns(file_path: str, patterns: List[str], project_root: str = None) -> str:
    """
...
class PythonCodeAnalyzer:
    ...
    def analyze(self, tree: ast.AST, content: str) -> Dict[str, Any]:
        ...
        if len(self.functions) > 20:
            insights.append("Large number of functions - consider splitting into modules")
        if any(length > 50 for length in function_lengths):
            insights.append("Some functions are very long - consider breaking them down")
...
def analyze_python_dependencies(content: str) -> Dict[str, List[str]]:
    ...
    try:
        ...
        # Categorize imports
        python_stdlib = {
            'os', 'sys', 'json', 'urllib', 'http', 'datetime', 'time', 're',
