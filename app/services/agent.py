...
class Agent:
    ...
    async def send_message(self, message: str, save_to_db: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Send message to AI
            ai_response = await self.ai_service.send_message(message, conversation_history)
            ...
            return {
                ...
                "usage": ai_response.get("usage", {}),
                "session_id": self.current_session.id if self.current_session else None,
                "message_id": ai_msg.id if save_to_db and self.current_session else None
            }
    ...
    async def send_system_message(self, system_prompt: str, user_message: str, save_to_db: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            ai_response = await self.ai_service.send_system_message(system_prompt, user_message)
    ...
    def get_session_messages(self, session_id: int = None) -> List[Dict[str, Any]]:
        ...
        return [msg.to_dict() for msg in sorted(session.messages, key=lambda x: x.created_at)]
