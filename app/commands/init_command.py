    ...
    async def execute(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return CommandResult(
                    False, f"Failed to analyze project: {str(e)}"
                ).to_dict()
...
Keep it concise but informative. Focus on making it useful for developers."""
            ...
            try:
                ...
                if not ai_response["success"]:
                    return CommandResult(
                        False,
                        f"AI documentation generation failed: {ai_response.get('error', 'Unknown error')}",
                    ).to_dict()
            ...
            try:
                ...
                if readme_result.startswith("Error"):
                    return CommandResult(
                        False, f"Failed to create README.md: {readme_result}"
                    ).to_dict()

            except Exception as e:
                return CommandResult(
                    False, f"Failed to create README.md: {str(e)}"
                ).to_dict()
...
"""
            ...
            return CommandResult(
                False, f"Init command failed: {str(e)}\n\nDetails: {error_detail}"
            ).to_dict()
